 ...
 */
package org.erian.modules.jamod.util;
...
public class ModbusSerialUtil {
	...
	public static Integer readData(SerialParams sParams, ReadParams request, String fCode) {
		Integer result = null;
		List<Integer> data = null;

		switch (fCode) {
		case "F01":
			data = readCoils(sParams, request);
			break;
		case "F02":
			data = readDiscretesInputs(sParams, request);
			break;
		case "F03":
			data = readRegisters(sParams, request);
			break;
		case "F04":
			data = readInputRegisters(sParams, request);
			break;
		default:
